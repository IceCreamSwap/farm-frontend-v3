...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed }) => {
  ...
  return (
    <FCard>
      {farm.tokenSymbol === 'GLTO' && <StyledCardAccent />}
      <CardImage>
      ...
      <Label>
        ...
        <span className="right">{farm.lpSymbol && farm.lpSymbol.toUpperCase().replace('GLTO', '')}</span>
      </Label>
      <Label>
        <span>{TranslateString(318, 'Earn')}</span>
        <span className="right">{farm.dual ? farm.dual.earnLabel : 'GLTO'}</span>
      </Label>
      {!removed && (
        <Label>
          <span>{TranslateString(736, 'APY')}</span>
